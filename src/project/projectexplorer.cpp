...
QStandardItem *ProjectExplorer::createItem(QString s,QStandardItem *p)
{
  QStandardItemModel * mod = qobject_cast<QStandardItemModel *>(model());
      ...
      if(p->load())
        {
          ...
          if(!p->hasSubProjects())
          ...
          else
            {
              ...
              foreach(Project * subProject,p->subProjects())
                {
                  if(subProject->load())
                    newProject(subProject->name(),projectName);
        ...
        {
          showMessage(tr("Failed to open project"),tr("The File does'nt exsits"));
        }
    }
  else
