...
void Options::setup()
{
  ...
  textEdit->setText(QLatin1String("int Foo(bool isBar) {\n"
                    "if (isBar) {\n"
                    "bar();\n"
                    "return 1;\n"
                    "} else\n"
...
void Options::setupCompilerSetting(CompilerSetting * cSetting)
{
  ...
  CompilerSettingWidget *compilerSettingWidget = new CompilerSettingWidget;
  compilerSettingWidget->setSetting(cSetting);

  compilerSettingWidget->setupCompilerOptions(new CompilerOptionsWidget(cSetting->toCompilerOptionsCategories()));
